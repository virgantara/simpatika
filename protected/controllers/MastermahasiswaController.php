...
// add a page
		...
		$pdf->Image($img_file, 0, 0, $pdf->getPageWidth(), 30);
		
		$no_ijazah = $model->no_ijazah ?: 'xxx/UNIDA/XXXX/X/XXX/'.date('Y');
		
		$margin_limit = 20;
		$pdf->SetAutoPageBreak($auto_page_break, $bMargin);
		$pdf->write2DBarcode($no_ijazah, 'QRCODE,Q', $pdf->getPageWidth() - 60, 45, 20, 20, $style, 'T');
		$style3 = array('width' => 0.1, 'cap' => 'butt', 'join' => 'miter', 'color' => array(255, 255, 255));
		$html = '<table border="0" cellspacing="3" cellpadding="4">';
		$html .= '<tr><td bgcolor="#D8D8D8" >Nomor: '.$no_ijazah.'</td></tr>';
		$html .= '</table>';
		$pdf->SetFont('dejavusans', '', 8);
		$pdf->writeHTMLCell(65, 10, $pdf->getPageWidth()-$margin_limit-55, 35, $html);
		...
		$pdf->Line($margin_limit-10, 35, $pdf->getPageWidth()-$margin_limit+10,35,$style);
		
		$pdf->SetFont('times', 'B', 20);
		$html = 'SURAT KETERANGAN<br>PENDAMPING IJAZAH';
		$pdf->writeHTMLCell(110, 10, $margin_limit, 35, $html);
		$pdf->SetFont('times', 'I', 14);
		$html = 'Diploma Supplement';
		$pdf->writeHTMLCell(110, 10, $margin_limit, 55, $html);

		$style = array('width' => 0.2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(100, 100, 100));
		$pdf->Line($margin_limit-10, 68, $pdf->getPageWidth()-$margin_limit+10,68,$style);
		...
		$pdf->SetFont('times', '', 8);
		$pdf->writeHTMLCell($pdf->getPageWidth()-2*$margin_limit, 10, $margin_limit, 70, $html);
		...
		$pdf->writeHTMLCell($pdf->getPageWidth()-2*$margin_limit, 10, $margin_limit, 82, $html);		
		
		$style = array('width' => 0.4, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(200, 200, 200));
		$pdf->Line($margin_limit, 95, $pdf->getPageWidth()-$margin_limit,95,$style);
		
		$pdf->SetFont('times', 'B', 7);
		$html = '1. INFORMASI TENTANG IDENTITAS DIRI PEMEGANG SKPI';
		$pdf->writeHTMLCell($pdf->getPageWidth()-2*$margin_limit, 10, $margin_limit, 97, $html);

		$pdf->SetFont('times', 'I', 7);
		$html = '1. Information Identifying the Holder of Diploma Supplement';
		$pdf->writeHTMLCell($pdf->getPageWidth()-2*$margin_limit, 10, $margin_limit, 100, $html);

		$pdf->SetFont('times', 'B', 7);
		$html = 'NAMA LENGKAP';
		$pdf->writeHTMLCell(($pdf->getPageWidth()-2*$margin_limit) / 2, 10, $margin_limit, 105, $html);

		$pdf->SetFont('times', 'I', 7);
		$html = 'Fullname';
		$pdf->writeHTMLCell(($pdf->getPageWidth()-2*$margin_limit) / 2, 10, $margin_limit, 108, $html);

		$html = '<table border="0" cellspacing="3" cellpadding="4">';
		$html .= '<tr><td bgcolor="#D8D8D8" >'.ucwords(strtolower($model->nama_mahasiswa)).'</td></tr>';
		$html .= '</table>';
		$pdf->SetFont('times', '', 8);
		$pdf->writeHTMLCell(($pdf->getPageWidth()-50) / 2, 10, $margin_limit-1, 112, $html);

		$pdf->SetFont('times', 'B', 7);
		$html = 'TAHUN LULUS';
		$pdf->writeHTMLCell(($pdf->getPageWidth()-2*$margin_limit) / 2, 10, $pdf->getPageWidth()  / 2 + 8, 105, $html);

		$pdf->SetFont('times', 'I', 7);
		$html = 'Year of Completion';
		$pdf->writeHTMLCell(($pdf->getPageWidth()-2*$margin_limit) / 2, 10, $pdf->getPageWidth() / 2 + 8, 108, $html);

		$html = '<table border="0" cellspacing="3" cellpadding="4">';
		$html .= '<tr><td bgcolor="#D8D8D8" >'.ucwords(strtolower($model->nama_mahasiswa)).'</td></tr>';
		$html .= '</table>';
		$pdf->SetFont('times', '', 8);
		$pdf->writeHTMLCell(($pdf->getPageWidth()-50) / 2, 10, $pdf->getPageWidth() / 2 + 7, 112, $html);

		$pdf->SetFont('times', 'B', 7);
		$html = 'TEMPAT DAN TANGGAL LAHIR';
		$pdf->writeHTMLCell(($pdf->getPageWidth()-2*$margin_limit) / 2, 10, $margin_limit, 123, $html);

		$pdf->SetFont('times', 'I', 7);
		$html = 'Date and Place of Birth';
		$pdf->writeHTMLCell(($pdf->getPageWidth()-2*$margin_limit) / 2, 10, $margin_limit, 126, $html);

		$html = '<table border="0" cellspacing="3" cellpadding="4">';
		$html .= '<tr><td bgcolor="#D8D8D8" >'.ucwords(strtolower($model->tempat_lahir.', '.$dob)).'</td></tr>';
		$html .= '<tr><td bgcolor="#D8D8D8" style="font-style:italic;">'.ucwords(strtolower($model->tempat_lahir.', '.date('d F Y',strtotime($model->tgl_lahir)))).'</td></tr>';
		$html .= '</table>';
		$pdf->SetFont('times', '', 8);
		$pdf->writeHTMLCell(($pdf->getPageWidth()-50) / 2, 10, $margin_limit-1, 129, $html);

		$pdf->SetFont('times', 'B', 7);
		$html = 'NOMOR IJAZAH';
		$pdf->writeHTMLCell(($pdf->getPageWidth()-2*$margin_limit) / 2, 10, $pdf->getPageWidth() / 2 + 8, 123, $html);

		$pdf->SetFont('times', 'I', 7);
		$html = 'Diploma Number';
		$pdf->writeHTMLCell(($pdf->getPageWidth()-2*$margin_limit) / 2, 10, $pdf->getPageWidth() / 2 + 8, 126, $html);
		...
		$html .= '</table>';
		$pdf->SetFont('times', '', 8);
		$pdf->writeHTMLCell(($pdf->getPageWidth()-50) / 2, 10, $pdf->getPageWidth() / 2 + 7, 129, $html);

		$pdf->SetFont('times', 'B', 7);
		$html = 'NOMOR INDUK MAHASISWA';
		$pdf->writeHTMLCell(($pdf->getPageWidth()-2*$margin_limit) / 2, 10, $margin_limit, 147, $html);

		$pdf->SetFont('times', 'I', 7);
		$html = 'Student Identification Number';
		$pdf->writeHTMLCell(($pdf->getPageWidth()-2*$margin_limit) / 2, 10, $margin_limit, 150, $html);

		$html = '<table border="0" cellspacing="3" cellpadding="4">';
		$html .= '<tr><td bgcolor="#D8D8D8" >'.ucwords(strtolower($model->nim_mhs)).'</td></tr>';
		$html .= '</table>';
		$pdf->SetFont('times', '', 8);
		$pdf->writeHTMLCell(($pdf->getPageWidth()-50) / 2, 10, $margin_limit-1, 153, $html);

		$pdf->SetFont('times', 'B', 7);
		$html = 'GELAR';
		$pdf->writeHTMLCell(($pdf->getPageWidth()-2*$margin_limit) / 2, 10, $pdf->getPageWidth() / 2 + 8, 147, $html);

		$pdf->SetFont('times', 'I', 7);
		$html = 'Name of Qualification';
		$pdf->writeHTMLCell(($pdf->getPageWidth()-2*$margin_limit) / 2, 10, $pdf->getPageWidth() / 2 + 8, 150, $html);

		$html = '<table border="0" cellspacing="3" cellpadding="4">';
		$html .= '<tr><td bgcolor="#D8D8D8" >'.ucwords(strtolower($model->kodeProdi->gelar_lulusan)).'</td></tr>';
		$html .= '<tr><td bgcolor="#D8D8D8" >'.ucwords(strtolower($model->kodeProdi->gelar_lulusan_en)).'</td></tr>';
		$html .= '</table>';
		$pdf->SetFont('times', '', 8);
		$pdf->writeHTMLCell(($pdf->getPageWidth()-50) / 2, 10, $pdf->getPageWidth() / 2 + 7, 153, $html);
