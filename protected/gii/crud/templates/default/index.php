...
echo "\$this->breadcrumbs=array(
	array('name'=>'".$this->class2name($this->modelClass)."','url'=>['index']),
	array('name'=>'Manage'),
);\n";
?>
	...
	$(document).ready(function(){
		$('#search,#size').change(function(){
			$('#<?php echo $this->class2id($this->modelClass); ?>-grid').yiiGridView.update('<?php echo $this->class2id($this->modelClass); ?>-grid', {
			    url:'<?php echo "<?php";?> echo Yii::app()->createUrl("<?php echo ($this->modelClass); ?>/index"); ?>&filter='+$('#search').val()+'&size='+$('#size').val()
			});
		});
...
?>
	...
	echo CHtml::link('Tambah Baru',array('<?php echo $this->modelClass; ?>/create'),['class'=>'btn btn-success']);
?></div>
