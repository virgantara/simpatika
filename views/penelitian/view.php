...
<div class="row">
    ...
    <div class="col-md-12">
        <div class="panel">
            <div class="panel-heading">
                <h1 class="panel-title">Peneliti</h1>
            </div>
            <div class="panel-body">
        ...
        <tbody>
        <?php 
        if(!empty($model->penelitianAnggotas))
        {
            foreach($model->penelitianAnggotas as $q=>$author)
            {
...
function getListAnggota(){
    ...
    var obj = new Object;
    obj.penelitian_id = "'.$model->ID.'";
   
    $.ajax({
        url: "'.Url::to(["penelitian/ajax-list-anggota"]).'",
        type: "POST",
        data: {
            dataPost: obj,
...
$(document).on("click",".btn-edit-author-save",function(e){
    ...
    $.ajax({
        url: "'.Url::to(["penelitian/ajax-update-author"]).'",
        type: "POST",
        data: {
            dataPost: obj,
...
$(document).on("click",".btn-remove-author",function(e){
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
            url: "'.Url::to(["penelitian/ajax-remove-author"]).'",
            type: "POST",
            data: {
                dataPost: obj,
...
$(document).on("click",".btn-save-author",function(e){
    ...
    $.ajax({
        url: "'.Url::to(["penelitian/ajax-add-author"]).'",
        type: "POST",
        data: {
            dataPost: obj,
