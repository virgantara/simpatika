...
class Generator extends \yii\gii\Generator
{
    ...
    public function generateActiveField($attribute)
    {
        ...
        if ($tableSchema === false || !isset($tableSchema->columns[$attribute])) {
            ...
            return "\$form->field(\$model, '$attribute',['options' => ['tag' => false]])";
        }
        $column = $tableSchema->columns[$attribute];
        if ($column->phpType === 'boolean') {
            return "\$form->field(\$model, '$attribute',['options' => ['tag' => false]])->checkbox()";
        }

        if ($column->type === 'text') {
            return "\$form->field(\$model, '$attribute',['options' => ['tag' => false]])->textarea(['rows' => 6])";
        }
        ...
        if (is_array($column->enumValues) && count($column->enumValues) > 0) {
            ...
            return "\$form->field(\$model, '$attribute',['options' => ['tag' => false]])->dropDownList("
                . preg_replace("/\n\s*/", ' ', VarDumper::export($dropDownOptions)).", ['prompt' => ''])";
        }

        if ($column->phpType !== 'string' || $column->size === null) {
            return "\$form->field(\$model, '$attribute',['options' => ['tag' => false]])->$input()";
        }

        return "\$form->field(\$model, '$attribute',['options' => ['tag' => false]])->$input(['class'=>'form-control','maxlength' => true])";
    }
