...
use app\models\BidangKepakaranSearch;
...
class BidangKepakaranController extends Controller
{
    ...
    public function actionGetBidangKepakaran()
    {
        $out = [];
        if (isset($_POST['depdrop_parents'])) {
            ...
            if ($parents != null) {
                $cat_id = $parents[0];
                $out = self::getListBidangKepakaran($cat_id); 
    ...
    private function getListBidangKepakaran($id)
    {
        $list = BidangKepakaran::find()->where(['parent'=>$id])->orderBy(['nama'=>SORT_ASC])->all();
